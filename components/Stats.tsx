...
export default function Stats() {
  ...
  return (
    <section className='pt-[29px] z-10 relative'>
      <div className='sm:rounded-[20px] sm:shadow-[0_0_0_0.5px_#ffffff47] sm:px-10 sm:py-6 sm:bg-[#FFFFFF0F]'>
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
          {stats.map((stat, index) => (
            <div
              key={index}
              className='flex flex-col gap-1 items-center rounded-[20px] shadow-[0_0_0_0.5px_#ffffff47] px-4 h-[66.32px] bg-[#FFFFFF0F] sm:px-0 sm:py-0 sm:rounded-none sm:shadow-none sm:bg-transparent sm:h-fit justify-center'
            >
              <h3 className='font-modernist sm:text-[36px] text-[30px] font-bold leading-[110%] tracking-[-0.87px] text-white'>
                {stat.value}
              </h3>

              <p className='font-dm-sans sm:text-[12px] text-[10px] font-normal leading-[110%] tracking-[-0.245px] text-white uppercase'>
                {stat.label}
              </p>
            </div>
          ))}
