  ...
  return (
    <div className="flex w-[388px] h-[507px] p-[11px] flex-col justify-center items-center gap-2 rounded-[20px] glass-border/28 bg-white/6">
      <div className="flex w-[366px] h-[480px] px-5 py-5 flex-col justify-between items-center flex-shrink-0 rounded-[20px] bg-gradient-to-b from-[#141414] to-[#141414]">
        ...
        {isWrongChain && (
          <div className="w-full mb-4 p-3 rounded-lg bg-yellow-500/20 border border-yellow-500/50">
            <div className="text-yellow-400 font-dm-sans text-[13px] font-medium leading-[120%] mb-2">
              ⚠️ Wrong Network Detected
            </div>
            <div className="text-white/80 font-dm-sans text-[12px] leading-[140%] mb-3">
              Your wallet is connected to <span className="font-semibold">{currentChainName}</span>, but this vault requires <span className="font-semibold">{requiredChainName}</span>.
            </div>
            <button
        ...
        <div className="flex h-[89px] px-[10px] py-[10px] flex-col justify-between items-center flex-shrink-0 self-stretch">
          ...
          <div className="flex items-center justify-between self-stretch">
            ...
            <div className="text-white font-dm-sans text-[13px] font-normal leading-[110%] tracking-[-0.256px]">
              {isLoadingSharePrice ? '...' : sharePrice || '1.00'} {vault?.underlying.symbol || '--'}
            </div>
          </div>
        ...
        <button
          ...
          disabled={isDepositing || isTxPending || isWrongChain || !amount || parseFloat(amount) <= 0 || !balanceFormatted || parseFloat(amount) > parseFloat(balanceFormatted)}
        ...
        >
          {isDepositing || isTxPending ? 'Processing...' : isWrongChain ? `Switch to ${requiredChainName} to Deposit` : 'Deposit'}
        </button>
      </div>
    </div>
  );
